...
public:
    ...
    static int count_edges(ListGraph& graph);

    // 2. Contar vértices adyacentes de un vértice dado
    static int count_adjacent_vertices(ListGraph& graph, Vertex v);

    // 3. Verificar si un grafo es conexo usando DFS
    static bool is_connected_dfs(ListGraph& graph);

    // 4. Verificar si un grafo es conexo usando BFS
    static bool is_connected_bfs(ListGraph& graph);

    // 5. Algoritmo de Dijkstra
    static void dijkstra(ListGraph& graph, Vertex source, double* distances);

    // 6. Algoritmo de Floyd-Warshall
    static void floyd_warshall(ListGraph& graph, double** distances);

    // 7. Caminos más cortos entre pares de vértices usando Dijkstra
    static void all_pairs_dijkstra(ListGraph& graph, double** distances);

    // 8. Algoritmo de Prim para árbol de expansión mínima
    static void prim(ListGraph& graph, int* parent);
    ...
    static void kruskal(ListGraph& graph, std::vector<EdgeDetail>& mst);

    // 10. Circuito Hamilton de menor costo (búsqueda exhaustiva)
    static double hamiltonian_path(ListGraph& graph, std::vector<int>& best_path);
};
